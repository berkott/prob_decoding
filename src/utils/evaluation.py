...
from utils.get_data import threshold_y
...
def evaluate_recording_classification(recording, X_test_recording, Y_test_recording, model, loss_fn, plot_num=10):
    # Test the model
    model.eval()
    ...
    for t in range(plot_num):
        ...
        fig.add_trace(go.Scatter(y=Y_test_recording[t], name='Actual'))
        fig.add_trace(go.Scatter(y=Y_pred[t].detach().numpy(), name='Predicted'))
        fig.add_trace(go.Scatter(y=threshold_y(Y_pred[t].detach().numpy(), 0.5), name='Predicted Thresholded'))
        fig.update_layout(title=f'Actual and Predicted Wheel Speeds, Recording {recording}, Trial {t}', xaxis_title='Time', yaxis_title='Value')
        fig.show()
